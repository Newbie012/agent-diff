...
import { fixtures, type BranchFixture, type FileFixture } from "./fixtures.ts"
...
export const createWorkspace = async (count: number): Promise<Workspace> => {
  const root = await mkdtemp(join(tmpdir(), "adiff-sim-"))
  ...
  const chosen: ReadonlyArray<BranchFixture> = fixtures.slice(0, Math.max(1, count))
  ...
  return { root, repo, storeRoot, branches, dispose: () => rm(root, { recursive: true, force: true }) }
}
