...
5. **Write the failing test first.** It asserts what a reviewer or an agent can observe. Run it and
   ...
   prints, names or exits. `adiff/test-title` checks the shape and the subject slot; it cannot check
   the voice, which is what CLAUDE.md's **How to write here** is for.
