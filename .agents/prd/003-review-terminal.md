...
| Before you send | `C` opens it on somebody else's pull request · `j`/`down`, `k`/`up` move · `e` rewords · `X` drops · `i` asks your agent to redraft · `ctrl+s` sends · `esc` returns |
...
- **A reviewer with no editor found is offered the ones on their machine, not a file to edit.**
  ...
  configure a tool. Escape closes the list and opens nothing. Changing the editor afterwards is a once-a-machine act, so it lives in the
