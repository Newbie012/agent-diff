...
fix(footer): a key that toggles something says which way it will go.
...
fix(layers): `L` asks for a reading order about the branch, not about the line the cursor happened to be on.
...
fix(layers rail): every changed file has a place in the rail, including one with no changed lines.
...
fix(footer): `→ N columns cut off` counts only columns that are cut, and names the key that pans.
